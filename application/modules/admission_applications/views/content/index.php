<div class="admin-box">
	...
	<ul class="nav nav-tabs" >
		<li <?php echo $filter=='' ? 'class="active"' : ''; ?>><a href="<?php echo $current_url; ?>" title='List of all current students expect spill-overs'>All Students</a></li>
		<li class="<?php echo $filter == 'studyMode' ? 'active ' : ''; ?>dropdown">
			<a href="#" class="dropdown-toggle" data-toggle="dropdown">
				By Study Mode <?php echo isset($filter_studymode) ? ": $filter_studymode" : ''; ?>
				<b class="caret light-caret"></b>
			</a>
			<ul class="dropdown-menu">
			<?php foreach ($studyModes as $key=>$name) : ?>
				<li>
					<a href="<?php echo $current_url .'?filter=studyMode&key='. $key ?>">
						<?php echo $name; ?>
					</a>
				</li>
			<?php endforeach; ?>
		...
		<li class="<?php echo $filter == 'faculty' ? 'active ' : ''; ?>dropdown">
			<a href="#" class="dropdown-toggle" data-toggle="dropdown">
				By Faculty <?php echo isset($filter_faculty) ? ": $filter_faculty" : ''; ?>
				<b class="caret light-caret"></b>
			</a>
			<ul class="dropdown-menu">
			<?php foreach ($faculties as $faculty) : ?>
				<li>
					<a href="<?php echo $current_url .'?filter=faculty&fac_id='. $faculty->fac_id ?>">
						<?php echo $faculty->fac_name; ?>
					</a>
				</li>
			<?php endforeach; ?>
			</ul>
	...
	<table class="table table-striped">
		<thead>
			...
			<tr>
				...
				<th>Matric</th>
				<th><?php echo lang('user_fullname'); ?></th>
		...
		<?php if (isset($students) && is_array($students) && count($students)) : ?>
			...
			<tr>
				<td>
					<input type="checkbox" name="checked[]" value="<?php echo $student->studentID ?>" />
				</td>
				<td><a href='<?php echo site_url(SITE_AREA.'/content/student_manager/edit/'.$student->studentID); ?>' title='Manage student academic details'>
					<?= $student->matricNo; ?>
				</a></td>
				<td><a href='<?php echo site_url(SITE_AREA.'/settings/users/edit/'.$student->user_id); ?>' target='_blank' title='Manage student personal details'>
					<?= strtoupper($student->lastname).', '.$student->firstname.' '.$student->middlename; ?>
				</a></td>
				<td colspan='2'><?php if (isset($student->prog_id)) {
						$progs = $this->programme_model->find_all_by('prog_id', $student->prog_id);
