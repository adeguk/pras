...
class Department_Model extends MY_Model {
	...
	protected $validation_rules 		= array(
		...
		array(
	    	'field' => 'dept_code',
	    	'label' => 'lang:pras_field_departmentCode',
	    	'rules' => 'required|trim',
	    ),
	    array(
	    	'field' => 'fac_id',
    ...
    public function department_list() {
        $depts = $this->select('dept_id, dept_name')->order_by('dept_name', 'ASC')->find_all_by('department.deleted', 0);
        $departments = array();
    ...
    */
    public function find_all() {
        if (empty($this->selects)) {
            $this->select($this->table_name .'.*, fac_name, firstname, middlename, lastname')->order_by('dept_name', 'ASC');
        }

        $this->db->join('faculty', 'faculty.fac_id = department.fac_id', 'left');
        $this->db->join('users', 'users.id = department.hod', 'left');
