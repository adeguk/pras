...
class Users extends Front_Controller {
    ...
    public function profile() {
        $this->load->model(array('basic_settings/education_model','contacts_view_model'));

        Assets::add_module_js('users','profile.js');
        Assets::add_module_css('users', 'profile.css');
		...
		$myContact = $this->contacts_view_model->find_all_by('user_id', $this->current_user->id);
		// get the current user educational information
		$myEducation = $this->education_model->order_by('endDate', 'DESC')->find_all_by('user_id', $this->current_user->id);

        Template::set('user', $user);
		Template::set('myContact', $myContact);
		Template::set('myEducation', $myEducation);
    ...
    public function user_education() {
        $this->load->model('basic_settings/education_model');
    	$this->load->helper('date');
        ...
        if (isset($id)) {
            $post = $this->education_model->limit(1)->find($id);
            Template::set('post', $post);
        }

        if ($this->input->post('submit') && isset($id)) {
            if ($this->save_education('update', $id)) {
                # Set current user's id
				$user = $this->user_model->find($user_id);

                # Log the Activity
				$log_name = (isset($user->display_name) && !empty($user->display_name)) ? $user->display_name : (
                            $this->settings_lib->item('auth.use_usernames') ? $user->username : $user->email);
				log_activity($this->current_user->id, lang('us_log_edit_profile') . ': ' . $log_name, 'users');

                # Set response message
                Template::set_message(lang('us_profile_updated_success'), 'success');
				# Redirect to same page upon success
				Template::redirect('/users/user_education');
            }
        ...
        Template::set_view('users/users/user_education');
        Template::render();
    } # end profile: user_education()

    /**
     * Allows a user to edit their own profile contact details.
     * @access public
     * @return void
     */
    public function contact_detail() {
        $this->load->model(array(
            'contacts_view_model',
            'basic_settings/state_model',
            'basic_settings/country_model',
            'lga_view_model'));
        ...
        # Set how to get variable id
        $id = $this->uri->segment(3);
        # Check if 'id' is set, if yes pre-fill form for edit
        if (isset($id)) {
            $this->load->model('basic_settings/contact_detail_model');
            $post = $this->contact_detail_model->limit(1)->find($id);
            Template::set('post', $post);
        }
        ...
        if ($this->input->post('submit') && isset($id)) {
            if ($this->save_contact('update', $id)) {
                ...
                Template::set_message(lang('us_profile_updated_success'), 'success');
				# Redirect to same page upon success
				Template::redirect('/users/contact_detail');
            }
        } elseif ($this->input->post('submit')) {
        ...
        $myContact = $this->contacts_view_model->find_all_by('user_id',$user_id);
        $listCountry = $this->country_model->country_list();
        $listState = $this->state_model->state_list();
        $listLga = $this->lga_view_model->lga_list();
        Template::set('listLga', $listLga);
        Template::set('listState', $listState);
        Template::set('listCountry', $listCountry);
        Template::set('records', $myContact);
        Template::set_view('users/users/contact_detail');
        Template::render();
    }//end profile: contact detail()
	 ...
	 * @return array
	 */
    private function save_education($type='insert', $id=NULL) {
        $this->auth->restrict();
        $this->set_current_user();
		...
		# Simply check to make the posted id is equal to the current user's id, minor security check
		/*if ( $_POST['id'] != $this->current_user->id ) 	{
			$this->education_model->form_validation->set_message('email', 'lang:us_invalid_userid');
			return FALSE;
		}*/

		// Validate the data
        $this->education_model->form_validation->set_rules($this->education_model->get_validation_rules());

        if ($this->education_model->form_validation->run() === false) {
             return false;
        }
        ...
        if ($type == 'insert') {
            ...
            $return = $this->education_model->update($id, $data);
        }

        return $return;
    } #end of save_education()

    private function save_contact($type='insert', $id=NULL) {
        $this->auth->restrict();
        $this->set_current_user();
        ...
        /*if ( $_POST['id'] != $this->current_user->id ) 	{
            $this->contact_detail_model->form_validation->set_message('email', 'lang:us_invalid_userid');
            return FALSE;
        }*/

        // Validate the data
        $this->contact_detail_model->form_validation->set_rules($this->contact_detail_model->get_validation_rules());

        if ($this->contact_detail_model->form_validation->run() === false) {
             return false;
        }
        ...
        $data = array(
            'user_id'           => $this->input->post('user_id'),
            'street_line1'		=> $this->input->post('street_line1'),
            'street_line2'		=> $this->input->post('street_line2'),
        ...
        if ($type == 'insert') {
            $return = $this->contact_detail_model->insert($data);
        }
        else {  // Update
            $return = $this->contact_detail_model->update($id, $data);
        }
