...
<!--Course name-->
    <div class="control-group <?php if (form_error('courseName')) echo 'error'; ?>">
		<?php echo form_label(lang('pras_field_course') . lang('bf_form_label_required'), 'courseName', array('class' => 'control-label')); ?>
        <div class="controls">
            <input type="text" name="courseName" class="input-xxlarge" placeholder="Type Course name..."
                value="<?php echo set_value('courseName', isset($post) ? $post->courseName : ''); ?>" />
...
<!--department name-->
	<div class="control-group <?php if (form_error('dept_id')) echo 'error'; ?>">
		<?php echo form_label(lang('pras_field_department') . lang('bf_form_label_required'), 'dept_id', array('class' => 'control-label')); ?>
		<div class="controls">
            <select name="dept_id" class="input-xxlarge chzn-select">
                <option value="0" >Please select the right Department...</option>
                <?php selectOption($listDepartment, $post->dept_id); ?>
            </select>
...
<!--status-->
	...
	<div class="form-actions">
		...
		<a href="<?php echo site_url(SITE_AREA .'/settings/course/courseBank') ?>" class="btn btn-warning">Cancel</a>
