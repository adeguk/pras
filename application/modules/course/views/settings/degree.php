...
<div class="admin-box">
    <h3>List of <?php echo lang('pras_field_degree'); ?></h3>
    ...
    <table class="table table-striped table-hover table-bordered">
        ...
        <?php if ($has_records) :
            foreach ($records as $record) :
                ...
                if ($can_delete) : ?>
			    <td><input type="checkbox" name="checked[]" value="<?php echo $record->deg_id ?>" /></td>
                <?php endif;

                if (($can_edit) && ($record->deleted == 1)) : ?>
                <td>
                    <?php echo anchor($areaUrl . 'editDegree/' . $record->deg_id, $edit_icon.$record->degreeName); ?>
                </td>
                <?php elseif ($can_edit) : ?>
                <td>
                    <?php echo anchor($areaUrl . 'editDegree/' . $record->deg_id, $edit_icon.$record->degreeName); ?>
                ...
                <?php else : ?>
                <td><?php e($record->degreeName); ?></td>
                <?php endif; ?>

				<td><?php e($record->degreeAbbreviation); ?></td>
				<td>
