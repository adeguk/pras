<?php
	Assets::add_css( array('bootstrap.css',	'bootstrap-responsive.css',	'loggTheme.css',));
						...
						<div class="nav pull-right" id="user-menu">
							<div class="btn-group">
								<a href="<?php echo site_url(SITE_AREA .'/settings/users/edit') ?>" id="tb_email" class="btn dark" title="<?php echo lang('bf_user_settings') ?>">
									<?php echo (isset($current_user->email) && !empty($current_user->email)) ? strtoupper($current_user->lastname).', '.$current_user->firstname .' '.$current_user->middlename : ($this->settings_lib->item('auth.use_usernames') ? $current_user->username : $current_user->email); ?>
								</a>
									...
									<li>
										<div class="inner">
											...
											<div class="toolbar-profile-info">
												<p><b><?php echo strtoupper($current_user->lastname).', '.$current_user->firstname .' '.$current_user->middlename ?></b><br/>
